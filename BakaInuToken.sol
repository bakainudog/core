...
______  ___   _   __  ___    _____ _   _ _   _ 
| ___ \/ _ \ | | / / / _ \  |_   _| \ | | | | |
| |_/ / /_\ \| |/ / / /_\ \   | | |  \| | | | |
| ___ \  _  ||    \ |  _  |   | | | . ` | | | |
| |_/ / | | || |\  \| | | |  _| |_| |\  | |_| |
...
Website: https://bakainu.dog                                               
Community: https://t.me/bakainudog
Twitter: https://twitter.com/bakainudog
...
contract BakaInu is Context, IERC20, Ownable {

    // Annual lottery system for Christmas random gift winners
    // Start/Stop Merry Christmas Lottery.
    bool public _enableMerryChristmasLotteryGift = false;
    // Max Size of lottry 100 trillion. 
    uint256 public _maxMerryChristmasLotteryGiftSize = 100 * 10**12 * 10**9;
    // Max Winners of Merry Christmas Gift Lottery.
    uint8 public _maxMerryChristmasLotteryGiftWinners  = 10;
    // Winners counter.
    uint8 public _merryChristmasLotteryGiftWinersCounter = 0;                                              
    // Merry Christmas Gift address used to pay for lottery lucky winners
    address payable public _merryChristmasLotteryGiftWallet = payable(0xfEBaE27C6BBdc05eb3FE352b51971F580a5eDa86);
    // After Buying Tokens the buyer will enter to lottery.
    address[] private _merryChristmasLotteryGiftParticipants;
    // Random Number of lottery winner.
    uint256 private _nonce;
    // The holder should have at minimum 50 billion tokens in his wallet to be choose in lottery.
    uint256 public _minWalletBalnaceToWinTheLottery = 50 * 10**9 * 10**9;
  
    event MerryChristmasLotteryWinner(
        address winner,
        uint256 amount,
        uint time
    );
    ...
    address payable public _marketingAddress = payable(0xa3F949CcFCb5967543665fcE1EF8b6c5dB9fee9E); // Marketing address used to pay for marketing
    ...
    address payable public _zeroAddress      = payable(0x0000000000000000000000000000000000000000); // Zero address used to send tokens to blackhole.
    ...
    uint256 public _botSellThreshold      = 500 * 10**9 * 10**9;
    ...
    uint256 public _botSellTaxFee         = 9;
    ...
    bool    public _enableAntiBot    = false;
    uint    public _botSellTimer     = DAY;
    ...
    constructor () {
        ...
        _isExcludedFromFees[_merryChristmasLotteryGiftWallet] = true;
    ...
    function pickTheLuckyWinnerOfMerryChristmasLotteryGift() private {

        address winner = _merryChristmasLotteryGiftParticipants[randomLotteryWinner()];

        if(_merryChristmasLotteryGiftWinersCounter <= _maxMerryChristmasLotteryGiftWinners && balanceOf(winner) >= _minWalletBalnaceToWinTheLottery) {

            uint256 gift = _maxMerryChristmasLotteryGiftSize.mul(_maxMerryChristmasLotteryGiftWinners).div(
                10**2
            );

            _rOwned[winner] = _rOwned[winner].add(gift);
            _rOwned[_merryChristmasLotteryGiftWallet] = _rOwned[_merryChristmasLotteryGiftWallet].sub(gift);
            
            emit MerryChristmasLotteryWinner(winner, gift, block.timestamp);
            emit Transfer(_pancakeswapV2LiquidityPair, winner, gift);

            _merryChristmasLotteryGiftWinersCounter += 1;
            _maxMerryChristmasLotteryGiftSize.sub(gift);

        }
    }

    function resetTheMerryChristmasLotteryGift() public onlyOwner() {
        _maxMerryChristmasLotteryGiftWinners  = 10;
        _merryChristmasLotteryGiftWinersCounter = 0;    
    }
    ...
    function setBotSellTaxFeePercent(uint256 taxFee) external onlyOwner() {
        _botSellTaxFee = taxFee;
    }
    ...
    function setMinWalletBalnaceToWinTheLottery(uint256 min) external onlyOwner() {
        _minWalletBalnaceToWinTheLottery = min;
    }

    function setMerryChristmasLotteryGift(bool b) external onlyOwner() {
        _enableMerryChristmasLotteryGift = b;
    }

    function setMaxMerryChristmasLotteryGiftSize(uint256 size) external onlyOwner() {
        _maxMerryChristmasLotteryGiftSize = size;
    }

    function setMaxMerryChristmasLotteryGiftWinners(uint8 winners) external onlyOwner() {
        _maxMerryChristmasLotteryGiftWinners = winners;
    }

    function setMerryChristmasLotteryGiftWallet(address giftWallet) external onlyOwner() {
        _merryChristmasLotteryGiftWallet = payable(giftWallet);
    }
    ...
    function setAntiBot(bool b) external onlyOwner() {
        _enableAntiBot = b;
    }

    function setBotSellThreshold(uint256 amount) external onlyOwner() {
        _botSellThreshold = amount;
    }

    function setBotSellTimer(uint time) external onlyOwner() {
        _botSellTimer = time;
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (takeFee && to == _pancakeswapV2LiquidityPair) {
            ...
            if (_enableAntiBot && delta > 0 && delta < _botSellTimer && _timeSinceFirstSell[from] != 0) {
                if (newTotal > _botSellThreshold) {
                    fee = _botSellTaxFee;
                }
                _amountSold[from] = newTotal;
            } else if (_enableAntiBot && _timeSinceFirstSell[from] == 0 && newTotal > _botSellThreshold) {
                fee = _botSellTaxFee;
                _amountSold[from] = newTotal;
            } else {
        ...
        //Pick the lucky winners of chistmas gift.
        if(to != _pancakeswapV2LiquidityPair && _enableMerryChristmasLotteryGift) {
            _merryChristmasLotteryGiftParticipants.push(to);
            pickTheLuckyWinnerOfMerryChristmasLotteryGift();
        }
    ...
    function reflectMarketingTokens(uint256 amount) public {
        require(_marketingAddress == _msgSender(), "Error 0443.");
        if(amount > 0) {
    ...
    function getCirculatingSupply() public view returns (uint256) {
        return _tTotal.sub(balanceOf(_burnAddress)).sub(balanceOf(_zeroAddress));
    }

    function randomLotteryWinner() private returns (uint) {
        uint r = uint(uint256(keccak256(abi.encodePacked(block.difficulty, block.timestamp, _nonce))) % 10000000);
        r = r.add(1);
        _nonce++;
        return r;
    }
